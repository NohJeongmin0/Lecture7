...
constexpr bool is_safe_to_go(LightColor c) {
    ...
    if (c == LightColor::Green){return true;}
    else{return false;}
}
...
std::string describe(CLightRef tl) {
    ...
    s += "duration_sec: " + std::to_string(tl.duration_sec) + ", ";
    s += "pedestrianMode: ";
    if (tl.pedestrian_mode){s += "true";}
    else{s += "false";}
    return s;
    // advance function needs to change the value of state, so we should use a reference.
    // describe function only prints the value, and do not change the value. So we take a const reference.
}
